...
{
public:
    Simulation(CSVMarketDataTradesManager& marketDataManager,
        ...
        void(*replaced_order_callback)(OrderPtr order),
        void(*new_order_callback)(OrderPtr order),
        void(*md_trade_callback)(MDTradePtr order)): 
    ...
    {
        m_output_canceled_orders_queue(order);
    }
    ...
    void Run()
    {
        for (auto iter = m_marketDataManager.begin(); iter != m_marketDataManager.end(); ++iter)
        {
            ...
            iter->LocalTimestamp = iter->EventTimestamp + m_marketDataLatency;
            m_output_md_trades_queue.PushBack(&(*iter));

            while (!m_input_order_queue.Empty() && 
                iter->EventTimestamp >= m_input_order_queue.Front()->CreateTimestamp + m_executionLatency)
            {
            ...
            while (!m_input_replaced_orders_queue.Empty() &&
                iter->EventTimestamp >= std::get<3>(m_input_replaced_orders_queue.Front()) + m_executionLatency)
            {
            ...
            while (!m_output_new_orders_queue.Empty() && 
                iter->EventTimestamp >= m_output_new_orders_queue.Front()->CreateTimestamp + m_executionLatency)
            {
            ...
            auto result = m_order_exection_manager.MatchWithPrice(iter->Price, iter->AggressorSide);
            ...
            while (!m_output_canceled_orders_queue.Empty() && 
                iter->EventTimestamp >= m_output_canceled_orders_queue.Front()->CreateTimestamp + 2*m_executionLatency)
            {
            ...
            while (!m_output_replaced_orders_queue.Empty() && 
                iter->EventTimestamp >= std::get<1>(m_output_replaced_orders_queue.Front()) + 2*m_executionLatency)
            {
            ...
            while (!m_output_executed_orders_queue.Empty() && 
                iter->EventTimestamp >= m_output_executed_orders_queue.Front()->CreateTimestamp + 2*m_executionLatency)
            {
            ...
            while (!m_output_md_trades_queue.Empty() && 
                iter->EventTimestamp >= m_output_md_trades_queue.Front()->LocalTimestamp)
            {
...
private:
    CSVMarketDataTradesManager& m_marketDataManager;
    ...
    void(*m_executed_order_callback)(OrderPtr order);
    void(*m_canceled_order_callback)(OrderPtr order);
    void(*m_replaced_order_callback)(OrderPtr order);
    void(*m_new_order_callback)(OrderPtr order);
    void(*m_md_trade_callback)(MDTradePtr order);
